...
export const TextEditor = forwardRef(
    (
        ...
        const handleAppendText = (html) => {
            if (!editorState) return

            const contentState = editorState.getCurrentContent()
            const selection = editorState.getSelection()

            const blocksFromHTML = window.htmlToDraft(html)

            if (!blocksFromHTML || !blocksFromHTML.contentBlocks) return

            // Convert contentBlocks to ContentState
            const contentStateFromHTML = ContentState.createFromBlockArray(
                blocksFromHTML.contentBlocks,
                blocksFromHTML.entityMap
            )

            // Insert new blocks into existing content
            ...
            const newHtml = window.stateToHTML(newContentState)
            setValue(questionKey, newHtml, { shouldValidate: true })
        }
        ...
        useEffect(() => {
            ...
            const loadEditorLibraries = async () => {
                try {
                    ...
                    window.convertFromHTML = convertFromHTML
